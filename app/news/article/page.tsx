...
// `description` verbatim before continuing. Strip that overlap so
// the reader isn't shown the same sentence twice in a row.
...
// breaks — group every 2 sentences into a paragraph so it reads
// like an article instead of a wall of text.
...
export default function ArticleDetailPage() {
    ...
    return (
      <div className="min-h-screen bg-[#08111e] text-white">
        ...
        <main className="mx-auto flex w-full max-w-[820px] flex-col items-center px-4 py-16 text-center sm:px-6">
          <Newspaper className="h-10 w-10 text-slate-600" strokeWidth={1.5} />
          <h1 className="mt-4 text-xl font-bold">Article not available</h1>
          <p className="mt-2 max-w-sm text-sm text-slate-400">
            This article couldn't be loaded — it may have opened in a new
            tab or the session expired. Head back to News and pick it again.
          </p>
          <Link
            href="/news"
  ...
  const snippet = cleanContent(article.content || article.description || "");
  const bodyParagraphs = toParagraphs(
    stripOverlap(article.description || "", snippet)
  );
      ...
      <main className="mx-auto w-full max-w-[820px] px-4 py-8 sm:px-6">
        ...
        <div className="mt-8 rounded-xl border border-white/10 bg-[#111c2d] p-5">
          <p className="text-sm text-slate-400">
            This is a preview from {article.source}. Read the full story on
            their site.
          </p>
